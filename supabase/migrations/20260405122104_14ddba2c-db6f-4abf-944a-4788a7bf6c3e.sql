...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'workshop_answers_person_id_fkey') THEN
    ...
    ADD CONSTRAINT workshop_answers_person_id_fkey
    FOREIGN KEY (person_id) REFERENCES public.people(id) ON DELETE CASCADE;
  END IF;

  -- workshop_answers -> workshop_questions
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'workshop_answers_question_id_fkey') THEN
    ALTER TABLE public.workshop_answers
    ADD CONSTRAINT workshop_answers_question_id_fkey
    FOREIGN KEY (question_id) REFERENCES public.workshop_questions(id) ON DELETE CASCADE;
  END IF;
END $$;
